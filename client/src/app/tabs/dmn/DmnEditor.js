...
import {
  convertDmnToCognitoIfRequired
} from '../util/cognitoConversion';
...
export const DEFAULT_ENGINE_PROFILE = {
  executionPlatform: ENGINES.COGNITO,
...
export class DmnEditor extends CachedComponent {
  ...
  async importXML(xml) {
    ...
    importedXML = await convertDmnToCognitoIfRequired(importedXML, onAction, onContentUpdated);
...
function getMigrationDialog() {
  return {
    ...
    detail: [
      'Only DMN 1.3 diagrams can be opened with Cognito Modeler v1.0.0 or later.',
    ].join('\n'),
