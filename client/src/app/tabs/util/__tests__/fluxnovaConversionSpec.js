...
import {
  convertBpmnToCognitoIfRequired,
  convertDmnToCognitoIfRequired, convertFormToCognitoIfRequired,
} from '../cognitoConversion';
import { spy } from 'sinon';
import { ENGINES, getLatestStable } from '../../../../util/Engines';

describe('tabs/bpmn/util - cognitoConversion', function() {
  ...
  const cognitoBpmn = `
    <?xml version="1.0" encoding="UTF-8"?>
    <bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:fluxnova="http://fluxnova.finos.org/schema/1.0/bpmn" id="Definitions_1ug79wa" targetNamespace="http://bpmn.io/schema/bpmn" exporter="Cognito Modeler" exporterVersion="5.33.0-dev" xmlns:modeler="http://fluxnova.finos.org/schema/modeler/1.0" modeler:executionPlatform="Cognito Platform" modeler:executionPlatformVersion="1.0.0">
        <bpmn:process id="Process_10ga7ii" isExecutable="true" >
        <bpmn:startEvent id="StartEvent_1" />
        </bpmn:process>
      <bpmndi:BPMNDiagram id="BPMNDiagram_1">
  ...
  const cognitoForm = {
    ...
    id: 'Form_0lvnpjd',
    exporter: {
      name: 'Cognito Modeler',
      version: '5.33.0-dev'
    },
    components: [],
    executionPlatform: 'Cognito Platform',
    executionPlatformVersion: '1.0.0'
  };

  describe('convertBpmnToCognitoIfRequired', function() {
    ...
    describe('should convert', function() {
      ...
      it('non cognito bpmn', async function() {

        const conversion = await convertBpmnToCognitoIfRequired(camundaNsBpmn, onAction, onContentChanged);
        ...
        expect(conversion).to.contain('modeler:executionPlatform="Cognito Platform"');
      ...
      it('non cognito dmn', async function() {

        const conversion = await convertDmnToCognitoIfRequired(camundaDmn, onAction, onContentChanged);
      ...
      it('non cognito form', async function() {

        const conversion = await convertFormToCognitoIfRequired(camundaForm, onAction);

        expect(onAction).to.have.been.calledWith('show-dialog');

        expect(conversion.executionPlatform).to.be.eql('Cognito Platform');
    ...
    describe('should not convert', function() {
      ...
      it('when model not converted', async function() {
        ...
        await convertBpmnToCognitoIfRequired(camundaNsBpmn, onAction, onContentChanged);
      ...
      it('when c8 xml detected', async function() {
        ...
        await convertBpmnToCognitoIfRequired(camunda8Bpmn, onAction, onContentChanged);
      ...
      it('when c8 form detected', async function() {
        ...
        await convertFormToCognitoIfRequired(camunda8Form, onAction);

        expect(onAction).to.have.been.calledWith('close-tab');
      });

      it('when xml contains both cognito ns and exe platform', async function() {

        let onContentChanged = spy();

        await convertBpmnToCognitoIfRequired(cognitoBpmn, onAction, onContentChanged);
      ...
      it('when xml entity invalid', async function() {
        ...
        await convertBpmnToCognitoIfRequired('some-entity', onAction, onContentChanged);
      ...
      it('when json contains both cognito ns and exe platform', async function() {

        await convertFormToCognitoIfRequired(cognitoForm, onAction);
      ...
      it('when json entity invalid', async function() {
        ...
        await convertFormToCognitoIfRequired('some-entity', onAction);
