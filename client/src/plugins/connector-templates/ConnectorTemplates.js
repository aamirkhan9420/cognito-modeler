  ...
  componentDidMount() {
    ...
    getGlobal('backend').on('client:connector-templates-update-success', (_, hasNew, warnings = []) => {
      ...
      if (warnings.length) {
        displayNotification({
          type: 'warning',
          title: 'Cognito Connector templates updated with warnings',
      ...
      displayNotification({
        type: 'success',
        title: hasNew ? 'Cognito Connector templates updated' : 'Cognito Connector templates up to date',
    ...
    getGlobal('backend').on('client:connector-templates-update-error', (_, message) => {
      displayNotification({
        type: 'error',
        title: 'Error updating Cognito Connector templates',
        content: <Fragment>
          <span>{ message }</span>
          <br />
          <a href={ DOCUMENTATION_URL }>Learn more</a>
