...
const expectedFiles = {
  win: [
    {
      name: 'cognito-modeler-${version}-win-${arch}.zip',
      archs,
      contents: [
        'Cognito Modeler.exe',
        'support/register_fileassoc.bat',
        'LICENSE.cognito-modeler.txt',
        'THIRD_PARTY_NOTICES.cognito-modeler.txt',
        'VERSION'
      ]
    }
  ],
  linux: [
    {
      name: 'cognito-modeler-${version}-linux-${arch}.tar.gz',
      archs: [ 'x64' ],
      contents: [
        'cognito-modeler-${version}-linux-${arch}/cognito-modeler',
        'cognito-modeler-${version}-linux-${arch}/support/xdg_register.sh',
        'cognito-modeler-${version}-linux-${arch}/VERSION'
      ]
    }
  ],
  mac: [
    {
      name: 'cognito-modeler-${version}-mac-${arch}.dmg',
      archs: [ 'x64', 'arm64' ]
    },
    {
      name: 'cognito-modeler-${version}-mac-${arch}.zip',
      archs: [ 'x64', 'arm64' ],
      contents: [
        'Cognito Modeler.app/Contents/Info.plist'
      ]
    }
  ]
};
